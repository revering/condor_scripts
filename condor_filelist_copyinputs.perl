...
#$executable=$ENV{"HOME"}."/bin/batch_cmsRun_copyInputs";
$rt=$ENV{"LOCALRT"};
$arch=$ENV{"SCRAM_ARCH"};
...
print OUTP "#!/bin/sh\n";
print OUTP $ENV{"HOME"}."/bin/batch_cmsRun_copyInputs \$@";
close(OUTP);
...
print(SUBMIT " && (Machine == \"scorpion46.spa.umn.edu\" || Machine == \"scorpion45.spa.umn.edu\")"); #|| Machine == \"scorpion43.spa.umn.edu\" )");# && Machine != \"scorpion17.spa.umn.edu\" && Machine != \"scorpion23.spa.umn.edu\" && Machine != \"scorpion21.spa.umn.edu\" && Machine != \"scorpion1.spa.umn.edu\" && Machine != \"scorpion6.spa.umn.edu\" && Machine != \"scorpion20.spa.umn.edu\" && Machine != \"scorpion44.spa.umn.edu\" && Machine != \"scorpion9.spa.umn.edu\" && Machine != \"scorpion5.spa.umn.edu\")");

#print(SUBMIT " && (Machine != \"scorpion42.spa.umn.edu\" && Machine != \"scorpion43.spa.umn.edu\" && Machine != \"scorpion45.spa.umn.edu\" && Machine != \"scorpion16.spa.umn.edu\" && Machine != \"scorpion18.spa.umn.edu\" && Machine != \"scorpion46.spa.umn.edu\")");
...
while ($i<=$#flist) {
    ...
    if($originalDir eq "")
    {
       print(SUBMIT "Arguments = $arch $rt $prodSpace/$jobBase $jobCfg $log $elog $fname $sleep none $cmsRunArguments\n");
    }
    else
    {
       print(SUBMIT "Arguments = $arch $rt $prodSpace/$jobBase $jobCfg $log $elog $fname $sleep $originalDir $cmsRunArguments\n");
    }
    print(SUBMIT "Queue\n");
}
        ...
        if ($sector==2 && /^[^\#]*fileName\s*=/) {
            ...
            for ($qq=0; $qq<=$#files; $qq++) {
                ...
                if ($storefile=~/store/) {
                   ...
                   if($originalDir eq "")
                   {
                      $storefile=~s|.*/$user/|file:/export/scratch/users/$user/$stub2/|;
                   }
                   else
                   {
                      $storefile=~s|.*$originalDir|file:/export/scratch/users/$user/$stub2/|;
                   }
                }
